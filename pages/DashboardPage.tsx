...
import { ScheduledProductionRun, Product, ProductionLine } from '../types';
...
const DashboardPage: React.FC = () => {
  const { 
    ...
    updateSchedule // Pass updateSchedule to GanttChart
  } = useAppData();
  ...
  const today = useMemo(() => {
    const now = new Date();
    // Set to midnight for consistent daily comparison
    now.setHours(0, 0, 0, 0);
    return now;
  }, []); // Recalculates only if something makes today change, effectively once per day render or if dependencies change
  ...
  const schedulesForTodayGantt = useMemo(() => {
    return schedules
      .filter(s => {
        const startTime = new Date(s.startTime);
        return startTime >= today && startTime < tomorrow;
      })
      .map(s => { // Ensure we have the product and line details for the Gantt chart
        ...
        return {
          ...
          lineOperatingHours: line?.operatingHours || []
        };
      });
  }, [schedules, today, tomorrow, getProductById, getProductionLineById]);


  const agendaDoDia = useMemo(() => {
    return schedulesForTodayGantt.sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime());
  },[schedulesForTodayGantt]);
  ...
  const linhasOcupadas = useMemo(() => {
    return schedules.filter(s => {
      ...
      return s.status === 'Em Progresso' && currentTime >= start && currentTime < end;
    });
  ...
  const handleGanttUpdateFeedback = (message: string, type: 'success' | 'error' | 'info') => {
    ...
    setTimeout(() => setGanttFeedback(null), 5000); // Clear feedback after 5 seconds
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Gantt Chart Section */}
      <Card title={`Planejamento de Produção - ${today.toLocaleDateString('pt-BR')}`}>
        {ganttFeedback && (
          <Alert type={ganttFeedback.type} message={ganttFeedback.message} onClose={() => setGanttFeedback(null)} />
        )}
        {schedulesForTodayGantt.length > 0 && productionLines.length > 0 ? (
          <GanttChart
            schedules={schedulesForTodayGantt}
            lines={productionLines.filter(line => schedulesForTodayGantt.some(s => s.lineId === line.id))} // Only pass lines that have schedules today
            day={today}
            currentTime={currentTime} // Pass current time to GanttChart
          ...
          <p className="text-green-600 py-4">
            {productionLines.length === 0 ? "Nenhuma linha de produção cadastrada." : "Nenhuma produção agendada para hoje."}
          </p>
        )}
      </Card>
