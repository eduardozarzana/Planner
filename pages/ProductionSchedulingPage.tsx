...
const ScheduleForm: React.FC<ScheduleFormProps> = ({
  ...
  useEffect(() => {
      ...
      setDurationCalculationWarning(`A linha "${line.name}" não possui equipamentos configurados. Duração será zero.`);
        ...
        if (totalDurationForOneUnit <= 0) {
          setDurationCalculationWarning("Atenção: A configuração atual de produto e linha resulta em uma duração de produção de zero minutos.");
           ...
           calculatedEndTimeObj = new Date(startDateObj.getTime() + totalDurationForAllUnitsMinutes * 60000);
           setDurationCalculationWarning(null); 
        }
    }
    ...
    const checkDateAgainstOperatingHours = (dateToCheck: Date, lineToCheck: ProductionLine, checkType: "início" | "fim"): string | null => {
        ...
        const opEndTimeMinutes = timeToMinutes(opDay.endTime);

        if (timeOfDayMinutes < opStartTimeMinutes || timeOfDayMinutes > opEndTimeMinutes) {
            if (checkType === "fim" && timeOfDayMinutes === opEndTimeMinutes) {
                 // Allow ending exactly at operating end time
            } else {
                 return `O horário de ${checkType} (${dateToCheck.toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'})} em ${daysOfWeekNamesShort[dayOfWeek]}) está fora do horário de operação da linha (${opDay.startTime} - ${opDay.endTime}).`;
            }
        }
        return null;
    };
    ...
    if (endOperatingHoursIssue && !(calculatedEndTimeObj.getTime() === startDateObj.getTime() && startOperatingHoursIssue === null)) {
        if (opHoursMsg) opHoursMsg += "\n"; 
        opHoursMsg += endOperatingHoursIssue + " ";
    }

    if (startDateObj.toDateString() !== calculatedEndTimeObj.toDateString()) {
        let currentDate = new Date(startDateObj);
        currentDate.setDate(currentDate.getDate() + 1); 

        while (currentDate < calculatedEndTimeObj && currentDate.toDateString() !== calculatedEndTimeObj.toDateString()) {
            const dayOfWeek = currentDate.getDay();
            const opDay = line.operatingHours.find(oh => oh.dayOfWeek === dayOfWeek);
            if (!opDay || !opDay.isActive) {
                const msg = `A produção atravessa ${daysOfWeekNamesShort[dayOfWeek]}, mas a linha não opera neste dia.`;
                if (opHoursMsg) opHoursMsg += "\n" + msg; else opHoursMsg = msg;
                break; 
            }
            currentDate.setDate(currentDate.getDate() + 1);
        }
    ...
    for (const existingSchedule of allSchedules) {
      if (existingSchedule.lineId === lineId) {
        ...
        // Stricter check: if product is Top Seller, no overlap allowed *at all* with its fixed times.
        ...
        if (currentStartTimeMs < existingEndTimeMs && existingStartTimeMs < currentEndTimeMs) {
          ...
          let conflictMsg = `Conflito! A linha "${lineDetails?.name || 'Desconhecida'}" já está ocupada ` +
                              ...
                              `${new Date(existingSchedule.startTime).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })} até ` +
                              `${new Date(existingSchedule.endTime).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}. `;
          if (currentProductIsTopSeller && existingProductIsTopSeller) {
            ...
            conflictMsg += `Ajuste o horário ou a linha.`;
          }
  ...
  }, [productId, lineId, startTime, quantity, getProductById, getProductionLineById, allSchedules, initialData]);
  ...
  const isSubmitDisabled = !!schedulingConflictWarning || !!operatingHoursWarning ||
                         (!!durationCalculationWarning && !durationCalculationWarning.includes("duração de produção de zero minutos") && endTime === '');
...
const ProductionSchedulingPage: React.FC = () => {
  const {
    ...
    optimizeDaySchedules, // Added from context
  ...
  const [pageError, setPageError] = useState<string | null>(null); // Renamed from addScheduleError
  ...
  return (
    <div className="space-y-6">
      ...
      {sortedSchedules.length === 0 ? (
        <Card>
           {!pageError && !optimizationResult && <p className="text-center text-green-500 py-8">Nenhuma produção agendada ainda. Clique em "Agendar Nova Produção" para começar.</p>}
           {(pageError || optimizationResult) && <p className="text-center text-green-500 py-8">Verifique as mensagens acima.</p>}
        </Card>
      ) : (
          ...
          <table className="min-w-full divide-y divide-green-200">
            ...
            <tbody className="bg-white divide-y divide-green-200">
              {sortedSchedules.map((item) => {
                ...
                const productClassificationLabel = product?.classification === 'Top Seller' ? ' (Top)' : ' (Nor)';
                ...
                return (
                  <tr key={item.id} className="hover:bg-lime-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-green-900">
                        {product?.name || 'N/D'}
                        <span className={`text-xs ${product?.classification === 'Top Seller' ? 'text-amber-600' : 'text-blue-600'}`}>
